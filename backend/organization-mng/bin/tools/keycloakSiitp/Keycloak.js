...
module.exports = {
  singleton() {
    if (!instance) {
      instance = new Keycloak({
        ...
        baseUrl: process.env.PINT_AUTH_URL,
        username: process.env.PINT_AUTH_USER_NAME,
        password: process.env.PINT_AUTH_USER_PASSWORD,
        grant_type: 'password',
        client_id: process.env.PINT_KEYCLOAK_BACKEND_CLIENT_ID,
      });
   ...
   * @returns {Keycloak}
   */
  multiton(organization, settings) {
    if (!multitonInstances[organization]) {
      multitonInstances[organization] = new Keycloak({
        realmName: settings?.pintRealmName || process.env.PINT_KEYCLOAK_BACKEND_REALM_NAME,
        baseUrl: settings?.pintAuthUrl || process.env.PINT_AUTH_URL,
        username: settings?.pintUser || process.env.PINT_AUTH_USER_NAME,
        password: settings?.pintPsw || process.env.KEYCLOAK_BACKEND_PASSWORD,
        client_id: settings?.pintClientId || process.env.PINT_KEYCLOAK_BACKEND_CLIENT_ID,
        grant_type: 'password'
      });
      ConsoleLogger.d(`tools.HSM.Keycloak MULTITON[${organization}] created pointing to multitonInstances[organization] ${settings?.pintBaseUrl || process.env.PINT_AUTH_URL}`);
    }
    return multitonInstances[organization];
  }
};
