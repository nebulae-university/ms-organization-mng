...
class KeyCloakDA {
    ...
    static createUser$(user, username) {
        ...
        return from(keycloak.keycloakClient.users.create(process.env.KEYCLOAK_BACKEND_REALM_NAME, userKeycloak).catch(err => { throw err })).pipe(
            ...
            catchError(({ statusCode, statusMessage, body }) => throwError(
                new CustomError(`KeycloakError: ${body ? body.errorMessage : statusMessage}`, 'KeyCloakDA.createUser$', 1, `statusCode:${statusCode}, statusMessage:${statusMessage}, errorMessage:${body ? body.errorMessage : ""}`)))
        );
    }
    ...
    static updateUser$(userId, user) {
        ...
        return from(keycloak.keycloakClient.users.update(process.env.KEYCLOAK_BACKEND_REALM_NAME, userKeycloak).catch(err => { throw err })).pipe(
            map(authUser => ([authUser, user])),
            catchError(({ statusCode, statusMessage, body }) => throwError(
                new CustomError(`KeycloakError: ${body ? body.errorMessage : statusMessage}`, 'KeyCloakDA.updateUser$', 1, `statusCode:${statusCode}, statusMessage:${statusMessage}, errorMessage:${body ? body.errorMessage : ""}`)))
        );
    }
    ...
    static deleteUsers$(userKeycloakId, user) {
        return from(keycloak.keycloakClient.users.remove(process.env.KEYCLOAK_BACKEND_REALM_NAME, userKeycloakId).catch(err => { (()=>{})(err); throw err; })).pipe(
            map(authUser => ([authUser, user])),
            catchError(({ statusCode, statusMessage, body }) => throwError(
                new CustomError(`KeycloakError: ${body ? body.errorMessage : statusMessage}`, 'KeyCloakDA.removeUser$', 1, `statusCode:${statusCode}, statusMessage:${statusMessage}, errorMessage:${body ? body.errorMessage : ""}`)))
        );
    }
