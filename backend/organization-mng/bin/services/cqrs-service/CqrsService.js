...
//const JWT_SECUNDARY_PUBLIC_KEY = process.env.JWT_SECUNDARY_PUBLIC_KEY.replace(/\\n/g, "\n");
            ...
            mergeMap(request => (request.failedValidations.length > 0)
              ...
              : of(request).pipe(
                ...
                mergeMap(({ authToken, message, handler }) =>
                  handler.fn.call(handler.instance, message.data, authToken, message).pipe(
 ...
 */
     verifyRequest$(aggregateType, request) {
      ...
      return of(request).pipe(
        ...
        map(message => {
          if (request.data.withoutAuth) {
            return { authToken: null, message, failedValidations: [] };
          } else {
            return ({ authToken: this.getJwtToken(request.data.jwt), message, failedValidations: [] });
          }
        }),
        ...
        tap(({ authToken }) => { 
          if (!request.data.withoutAuth && !RoleValidator.hasRoles(authToken.realm_access.roles, jwtValidation.roles || [])) throw permissionDeniedError 
        }),
