...
function TodoSidebarContent(props) {
    ...
    const { filters: { active: activeChecked } } = useSelector(({ OrganizationManagement }) => OrganizationManagement.organizations);
    ...
    function handleActiveChange(evt) {
        if(activeChecked === null){
            dispatch(Actions.setOrganizationFilterActive(true));
        }else if(activeChecked){
            dispatch(Actions.setOrganizationFilterActive(false));
        }else{
            dispatch(Actions.setOrganizationFilterActive(null));
        }        
    }
                ...
                <div className="p-24">
                    <FormGroup row>
                        <FormControlLabel
                            ...
                            label={T.translate("organizations.filters.active")}
                        />
                    </FormGroup>
                </div>
