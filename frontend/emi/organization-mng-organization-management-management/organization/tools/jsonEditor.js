...
export const CustomJsonEditor = props => {
    ...
    const { T, dataSource, setSpecsUpdated, handleSpecsUpdated, readOnly, setErrorValidate, tabValue } = props;
    ...
    function validateJson(errors, keys) {
        const regex =  tabValue === 1 ? /^[a-zA-Z]+[a-zA-Z0-9-_]*$/: /^([a-zA-Z0-9]+-?)*$/;
        const error = errors || [];
        ...
        keys.forEach(key => {
            if(!regex.test(key[0])){
                error.push({
                    path: ['warning'],
                    message: T.translate("organization.invalidKey", { key: key[0] })
                });
            }
