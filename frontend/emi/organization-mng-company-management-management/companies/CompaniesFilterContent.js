...
function TodoSidebarContent(props) {
    ...
    const { filters: { active: activeChecked } } = useSelector(({ CompanyManagement }) => CompanyManagement.companies);
    ...
    function handleActiveChange(evt) {
        if(activeChecked === null){
            dispatch(Actions.setCompaniesFilterActive(true));
        }else if(activeChecked){
            dispatch(Actions.setCompaniesFilterActive(false));
        }else{
            dispatch(Actions.setCompaniesFilterActive(null));
        }        
    }
                ...
                <div className="p-24">
                    <FormGroup row>
                        <FormControlLabel
                            ...
                            label={T.translate("companies.filters.active")}
                        />
                    </FormGroup>
                </div>
